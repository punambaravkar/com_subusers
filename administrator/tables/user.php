 ...
 * @since  1.6
 */
class SubusersTableuser extends JTable
{
	/**
	 * Constructor
	 *
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_subusers.user.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_subusers/access.xml',
				"/access/section[@name='user']/"
			);
			$default_actions = JAccess::getAssetRules('com_subusers.user.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_subusers.user.' . (int) $this->$k;
	}
